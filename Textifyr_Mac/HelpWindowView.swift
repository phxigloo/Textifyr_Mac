...
let helpTopics: [HelpTopic] = [
        ...
        content: HelpContent(
            ...
            uiViews: [
                ...
                HelpUIView(
                    name: "Action Editor",
                    icon: "wand.and.stars",
                    function: "The Action Editor (Tools → Action Editor, ⌘⇧E) is where you create, edit, and organise formatting actions. An action is a named sequence of one or more steps. Each step can run an AI prompt or a fast, no-AI text transform (see the Transform Steps topic).",
                    howItWorks: "Actions are organised into three scopes: After Capture (runs automatically after any source is captured), Before Combining (run manually on a single source), and Final Document (runs on all sources combined). Each action can have multiple steps that run in sequence. The app comes with a handful of suggested actions to get you started — every action is fully editable, so rename them, change their prompts, add or remove steps, or delete the ones you don't need. You can also copy steps from other actions, reorder them by dragging, and hide any action you don't use.",
                    appFlow: "Actions created in the Action Editor become available throughout the app — in the source detail action bar, in the output pane's action menu, and in the Auto Cleanup settings. The Action Editor is the central place to manage all your AI formatting instructions. When you write a new prompt in the Instruction Lab and save it, it is saved here as a new action step.",
                    buttons: "• + (footer) — Adds a new action in the selected scope.\n• − (footer) — Deletes the selected action.\n• Show hidden toggle — Reveals actions that have been hidden.\n• Add Step (action detail) — Adds a new prompt step to the selected action.\n• Copy Step from Pipeline — Opens a sheet to copy a step from any other action.\n• Save (⌘S) — Saves unsaved changes. A confirmation dialog appears if you try to navigate away with unsaved changes.\n• Discard — Reverts all unsaved changes to the selected action.\n• Run (step detail) — Tests the step's prompt against the built-in sample text.\n• Instruction Lab (step detail) — Opens the Instruction Lab pre-loaded with this step's prompt.",
                ...
                HelpUIView(
                    name: "Transform Steps (No AI)",
                    icon: "wrench.and.screwdriver",
                    function: "Every step in an action can be either an AI Prompt or a Transform. A Transform is a fast, exact text operation that runs entirely on your Mac — it never uses Apple Intelligence and is never affected by the content filter, so it produces the same result instantly, every time.",
                    howItWorks: "Use the type menu at the top-right of any step to switch what it does. Six transform types are available:\n• Find & Replace — swap exact text, or a regular-expression pattern, for something else. Type \\t for a Tab and \\n for a new line; toggle Case sensitive or Regular expression as needed.\n• Convert Delimiters — change one column separator into another: commas, vertical bars “|”, semicolons, or a “<SEP>” token to and from Tabs.\n• Clean Whitespace — normalise line endings, trim each line, collapse repeated spaces, and collapse multiple blank lines into one.\n• Change Letter Case — UPPERCASE, lowercase, Title Case, or Sentence case.\n• Line Tools — remove empty or duplicate lines, sort lines, and add a header row.\n• Fix Look-alike Characters — replace letters from other alphabets that look like Latin letters (for example a Cyrillic “о” or a Greek “Α”) with the plain Latin equivalent.",
                    appFlow: "Transforms run in sequence alongside AI steps — each step's output becomes the next step's input — so you can mix them freely. The most powerful pattern is to let AI do the thinking and a transform do the exact formatting the AI can't be trusted to get right. Example: an AI step pulls the Date, Payee, and Total from a receipt and writes them separated by a vertical bar (Date | Payee | Total); a Convert Delimiters step then turns each bar into a real Tab, giving a clean tab-separated row every time. The vertical bar is used on purpose — commas inside amounts like $1,200.00 would break a comma split, but bars never appear in the data.",
                    buttons: "• Step type menu (top-right of a step) — Switches the step between AI Prompt and each transform type.\n• Find / Replace fields — The text to search for and what to put in its place. Type \\t for a Tab, \\n for a new line.\n• Regular expression — Treats the Find field as a regex pattern; use $1, $2 for captured groups in Replace.\n• Case sensitive — Whether matching respects upper/lower case.\n• Conversion (Convert Delimiters) — Picks which separator becomes which.\n• Whitespace / Line Tools checkboxes — Turn each clean-up option on or off.\n• Header row (Line Tools) — Optional first line to add; type \\t between column names for a tab-separated header.",
                    tips: "• Reach for a transform whenever you've asked the AI to 'change every comma to a Tab', 'remove the blank lines', or 'make it all uppercase' and the result came out inconsistent — a transform does these perfectly, every time, and never trips the content filter.\n• Put a Fix Look-alike Characters transform before an AI step if your text came from a PDF, scan, or screenshot — it removes the stray foreign characters that sometimes cause the AI to block your text.\n• When you need exact, structured output (CSV, TSV, a fixed header row), do the thinking with AI and the formatting with transforms."
                ),
                HelpUIView(
                    name: "Checking & Retrying Results",
                    icon: "checkmark.shield",
                    function: "Any AI step can check its own output and automatically try again if it came out wrong. This catches the times Apple Intelligence returns an incomplete or mis-shaped result — so a bad run gets retried instead of silently saved.",
                    howItWorks: "Open any AI step in the Action Editor and switch on “Check the result, and retry if it's wrong.” Then pick a check:\n• Result is not empty — the step actually produced something.\n• Every line has N columns — each line has exactly the number of tab-separated columns you set (ideal for tables and TSV).\n• Contains these words — the output includes every word you list.\n• Matches a pattern — the output matches a regular-expression pattern.\nYou also set how many times to try, and what to do if it still fails. The checks run on your Mac with no AI, so they are instant and completely reliable.",
                    appFlow: "The check runs the moment a step finishes. If it fails, the step runs again — the AI is told what was wrong so it can fix it — up to the number of tries you chose. This pairs perfectly with extraction: for example, pull Date, Payee and Total from a receipt, then check “every line has 3 columns” so a malformed receipt is retried automatically rather than producing a broken row.",
                    buttons: "• Check the result toggle — Turns checking on for this step.\n• Check menu — Chooses which check to apply.\n• Each line has N columns stepper — Sets the expected column count (for the columns check).\n• Words / pattern field — The required words, or the regex pattern (for those checks).\n• Try up to N times stepper — How many attempts before giving up.\n• If it still fails menu — Use the result anyway and continue, or stop and tell you.",
                    tips: "• “Every line has N columns” is the most useful check for spreadsheet-style output — it guarantees clean rows or keeps retrying.\n• Keep the number of tries low (2–3); each retry runs the AI again and takes time.\n• Choose “Stop and tell me” for important data you'd rather fix by hand than accept slightly wrong."
                ),
                HelpUIView(
                    name: "Instruction Lab",
                    icon: "text.badge.plus",
                    function: "The Instruction Lab (Tools → Instruction Lab, ⌘⇧P) is an interactive workspace for writing and testing AI prompts. You write a prompt, run it against sample text, and refine it until the output is exactly right — then save it to an action.",
                    howItWorks: "The Instruction Lab has three columns: Samples (on the left), the Sample Work Area (centre), and the Prompt editor (right). Select a built-in sample or create your own, write a prompt in the right-hand panel, and click Run to see what Apple Intelligence produces. The AI Improvement chat panel slides in from the right to help you refine the prompt through conversation.",
                    appFlow: "The Instruction Lab is the development environment for your AI prompts. You use it to design and test prompts before saving them to an action step in the Action Editor. It sits outside the main document workflow — changes here do not affect any document until you explicitly save to an action.",
                ...
                HelpUIView(
                    name: "Settings",
                    icon: "gearshape",
                    function: "Settings (⌘,) is where you configure Textifyr's global behaviour: AI privacy options, network access, filler word lists, find & replace rules, a protected-names glossary, workflow stages, and window management.",
                    howItWorks: "Settings is divided into four tabs: General (AI and privacy options), Text Processing (filler words, cleanup rules, the glossary, and the default action), Stages (workflow stage management), and Windows (document window limits and quick links to the Action Editor and Instruction Lab).",
    ...
    HelpTopic(
        id: "prompt-builder",
        title: "Instruction Lab",
        icon: "text.badge.plus",
        content: HelpContent(sections: [
            HelpSection(heading: nil,
                body: "The Instruction Lab (Tools → Instruction Lab, or ⌘⇧P) helps you design and test AI action prompts interactively."),
